...
export interface User extends AuthenticatedUser {
  // Add other fields if needed/returned by /me or /login
}
  ...
  try {
    ...
    if (contentType && contentType.indexOf("application/json") !== -1) {
        ...
        if (response.ok) {
            ...
            try { data = JSON.parse(text); } catch { data = { message: text || 'Success' }; }
        } else {
...
export const register = (userData: { name?: string; email: string; password: string; role?: string }): Promise<{ message: string, user: User }> => {
...
export const getClientSessions = (): Promise<any[]> => { // Replace 'any' with Session type later
  return apiFetch<any[]>('/sessions/client', { method: 'GET' });
};

export const getClientResources = (): Promise<any[]> => { // Replace 'any' with Resource type later
