...
const mockResources = [
  {
    id: 'res1',
    title: 'Welcome Guide (Mock)',
    type: 'PDF',
    createdAt: new Date(Date.now() - 2 * 86400000).toISOString(),
    content: 'Welcome_Guide_Mock.pdf',
  },
  {
    id: 'res2',
    title: 'Goal Setting Worksheet (Mock)',
    type: 'DOCUMENT',
    createdAt: new Date(Date.now() - 1 * 86400000).toISOString(),
    content: 'Goal_Setting_Mock.docx',
  },
  {
    id: 'res3',
    title: 'Intro Video (Mock)',
    type: 'VIDEO',
    createdAt: new Date().toISOString(),
    content: 'Intro_Video_Mock.mp4',
  },
];
...
router.post(
  '/',
  /* isAuthenticated, isCoach, validation, */ (req: Request, res: Response) => {
...
router.delete(
  ...
  /* isAuthenticated, isCoach, validation, */ (req: Request, res: Response) => {
    ...
    const { userId, resourceId } = req.body;
    console.log(
      `Mock DELETE /resources/unassign hit: Unassign resource ${resourceId} from user ${userId}`
    );
    if (!userId || !resourceId) {
      return res.status(400).json({ message: 'Mock requires userId and resourceId' });
    }
