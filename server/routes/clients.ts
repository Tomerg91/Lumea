...
router.get('/', /* isAuthenticated, isCoach, */ (req: Request, res: Response, next: NextFunction) => {
  console.log("Mock GET /coach/clients hit");
  // Return a static list of mock clients
  res.json([
    { id: 'client1', name: 'Mock Client Alpha', email:'alpha@test.com', createdAt: new Date(Date.now() - 86400000).toISOString()}, // Yesterday
    { id: 'client2', name: 'Mock Client Beta', email:'beta@test.com', createdAt: new Date().toISOString()} // Today
  ]);
});

// POST /coach/clients - Mock: Create a new client (User with role CLIENT)
router.post('/', /* isAuthenticated, isCoach, clientCreationValidationRules, validate, */ (req: Request, res: Response, next: NextFunction) => {
...
router.put('/:clientId', /* isAuthenticated, isCoach, ... */ (req: Request, res: Response) => {
...
router.delete('/:clientId', /* isAuthenticated, isCoach, ... */ (req: Request, res: Response) => {
...
router.get('/:clientId', /* isAuthenticated, isCoach, ... */ (req: Request, res: Response) => {
